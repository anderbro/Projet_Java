...
public class Engine implements Runnable {
			...
			while (!this.partieFinie()) {
				if (this.hasPlayers())
					...
					synchronized (this.running) {
						// permet de marcher ca tourne pas en rond lol + synchronized pour un acc�s
						// exclusif sur le lock
						// pour synchro deux thread accedant aux memes donn�es
						this.running.wait();
					}
				}
			}
	...
	private void tourJoueur(int id) throws IOException {
		...
		try {
			...
			String reponse;
			do {
				j.message("Veuillez saisir 4 d�placements valides, parmis z q s d ");
				reponse = j.ecoute();
			...
			// On envoie � l'utilisateur le plateau � afficher
	...
	public boolean hasPlayers() {
		...
		for (int i = 0; i < this.joueurs.size(); i++) {
			...
			if (!p.isConnected()) {
				toDelete.add(p);
				System.out.println("Joueur " + p.getNom() + " d�connect�.");
			}
		}
	...
	public void move(Joueur j, Mouvement direction) {
		...
		// detecte la case sur laquelle le joueur se d�place et fait l'interaction
		// correspondante
		switch (res.getType()) {
		case Joueur:
